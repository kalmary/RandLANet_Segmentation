...
class Dataset(IterableDataset):
    ...
    def __init__(self, base_dir: Union[str, pth.Path],
        ...
        Args:
            ...
            oversample_factor: How much to oversample minority classes (1.0 = no change)
            augment_minority: If True, augment minority class samples
            minority_threshold: Classes below this fraction are considered minority
        """
    ...
    def _process_cloud(self):
        ...
        for cloud in self._key_streamer():
            ...
            # Apply class-balanced sampling (if enabled and training)
            if self.class_balanced_sampling and self.shuffle:
                cloud_tensor, labels_tensor = self._balance_point_cloud(
                    cloud_tensor, 
                    labels_tensor
                )
                features_tensor = features_tensor[:len(labels_tensor)]  # Match length

            # Standard augmentations (training only)
            if self.shuffle:
                cloud_tensor = cloud_tensor.to(self.device)
