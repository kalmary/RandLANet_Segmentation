...
def input_norm(input: torch.Tensor, max_voxel_dim: Optional[float] = None):
    if max_voxel_dim is None:
        return input

    max_voxel_dim /= 2 # voxels should be in range (-10, 10), intensity (0, 10) -> both moved into (0, .2) range

    input[..., :3].div_(max_voxel_dim / 2.0)  # (-10,10) → (-1,1)
    input[..., -1].div_(10.0)                  # (0,10) → (0,1)
    return input
...
class RandLANet(nn.Module):
    def __init__(self, model_config: dict, num_classes: int):
        ...
        fc_start_config = model_config.get('fc_start')
        fc_end_config = model_config.get('fc_end')
...
def test_model():
    ...
    model = RandLANet.from_config_file("src/final_files/RandLANet15.json", num_classes=10).to(device)
